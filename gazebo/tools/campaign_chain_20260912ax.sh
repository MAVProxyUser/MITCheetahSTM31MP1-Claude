#!/bin/bash
# Chain AX (2026-09-12 15:40): the validation tiers on the continuity-window
# follower (ISSUES OPEN-38), after chain AW (pid 38145). AW runs the fast
# tier once; the follower change touches every mission, so the FULL tier
...
PREV="${PREV_CHAIN_PID:-38145}"
LOG="$CAMPAIGN_DIR/open38_chainax.log"
...
say "chain AX pid $$: waiting for chain AW (pid $PREV) to exit"
while kill -0 "$PREV" 2>/dev/null && ps -p "$PREV" -o command= | grep -q "campaign_chain_20260912aw[.]sh"; do sleep 30; done
...
python3 unittests/test_validated_missions.py > "$CAMPAIGN_DIR/suite_full_20260912_trackwin.log" 2>&1
say "suite exit $?: $(grep -E "SUMMARY|passed|failed" "$CAMPAIGN_DIR/suite_full_20260912_trackwin.log" | tail -3 | tr '\n' ' ')"
say "chain AX done"
