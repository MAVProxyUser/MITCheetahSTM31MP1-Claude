#!/bin/bash
# Chain CI (2026-09-15 21:55): ship the debounce on the OTHER TWO locomotionSafe branches
...
# absorbed-spike lines and the trips in the block's archived ctrl logs, until STOP_CI.
...
# same, and the tier, DUMP=1, pooled scores each block, until STOP_CI (RULE ONE). If the
...
tier(){ local t="$1"; tm_wait; say "$t: fast suite tier"
  python3 unittests/test_validated_missions.py --fast > "$CAMPAIGN_DIR/suite_fast_20260915_ci$t.log" 2>&1
  say "$t suite exit $?: $(grep -cE "^  PASS" "$CAMPAIGN_DIR/suite_fast_20260915_ci$t.log") PASS, $(grep -cE "^  FAIL" "$CAMPAIGN_DIR/suite_fast_20260915_ci$t.log") FAIL"
  sleep 20; wait_idle
}
legv_count(){ local runs f trips=0 spikes=0 n=0
  ...
  for r in $runs; do f=$(ls "$DATA"/conductor/archive/*run"$r"_ctrl_0.log 2>/dev/null | head -1); [ -z "$f" ] && continue; n=$((n+1))
    grep -qE 'moving too quickly|y-position is bad|is above hip' "$f" && trips=$((trips+1)); s=$(grep -cE '^\[(legv|legkin)\]' "$f"); [ "$s" -gt 0 ] && spikes=$((spikes+1)); done
  echo "$n runs, $trips tripped, $spikes with an absorbed spike"; }
PREV="${PREV_CHAIN_PID:-10781}"
LOG="$CAMPAIGN_DIR/open39_chainci.log"
say "chain CI pid $$: waiting for chain CH (pid $PREV) to exit"
while kill -0 "$PREV" 2>/dev/null && ps -p "$PREV" -o command= | grep -q "campaign_chain_20260912ch[.]sh"; do sleep 30; done
rm -f "$CAMPAIGN_DIR/STOP_CI"
wait_idle; sleep 30; wait_idle; tm_wait
OLD=$(md5 -q host-run/mit_ctrl_sim | cut -c1-8)
say "deploy: building mit_ctrl_sim (y-position + above-hip debounce, CTRL_LEGY/HIP_TRIP_TICKS=5) - deployed binary now $OLD"
DEPLOYED=0
if cmake --build host-build -j2 --target mit_ctrl_sim > "$CAMPAIGN_DIR/build_ci.log" 2>&1; then
  say "build ok ($(grep -c 'warning' "$CAMPAIGN_DIR/build_ci.log") warnings); deploying through deploy_host.sh"
  if bash gazebo/deploy_host.sh >> "$CAMPAIGN_DIR/build_ci.log" 2>&1; then DEPLOYED=1; say "deploy ok: binary $OLD -> $(md5 -q host-run/mit_ctrl_sim | cut -c1-8)"; else say "DEPLOY FAILED (see build_ci.log) - the old binary stays"; fi
else
  say "BUILD FAILED (see build_ci.log) - the old binary stays"
fi
# probe the served recipe once and qualify on the snapshot + dump (archived at the run's end)
wait_idle; tm_wait
COURSES=wkc_finals ARMS="nodamp:" run wkc26_ld_ci_probe 1 2.6
Q=$(python3 - <<'PY'
import csv, sys
...
import os
rows = list(csv.DictReader(open(os.path.join(os.environ.get("CHEETAH_DATA", os.path.expanduser("~/Desktop/Cheetah/rundata")), "campaigns", "wkc26_ld_ci_probe.csv"))))
...
tier 0
i=0
while [ ! -f "$CAMPAIGN_DIR/STOP_CI" ]; do
  i=$((i+1)); wait_idle; sleep 30; wait_idle; tm_wait; say "block $i: rig idle (phase $(phase)) - binary $(md5 -q host-run/mit_ctrl_sim | cut -c1-8)"
  COURSES=wkc_finals ARMS="$A" run "wkc26_ld_ci$i" 6 2.6
  say "wkc lie-down, pooled:"; python3 gazebo/tools/liedown_peak.py "nodamp=wkc26_ld_c[fghi]*:nodamp" "stock_new=wkc26_ld_c[ghi]*:stock" "stock_all=wkc26_l*:recipe" | tee -a "$LOG"
  say "wkc hand-off joints (dumps):"; python3 gazebo/tools/liedown_handoff.py "nodamp=wkc26_ld_c[fghi]*:nodamp" "stock=wkc26_ld_c[ghi]*:stock" | tee -a "$LOG"
  [ -f "$CAMPAIGN_DIR/STOP_CI" ] && break
  COURSES=hp_gap20 ARMS="$A" run "hp26_ld_ci$i" 6 2.6
  say "hairpin lie-down, pooled:"; python3 gazebo/tools/liedown_peak.py "nodamp=hp26_ld_c[fghi]*:nodamp" "stock_new=hp26_ld_c[ghi]*:stock" "stock_all=hp26_l*:recipe" | tee -a "$LOG"
  say "hairpin hand-off joints (dumps):"; python3 gazebo/tools/liedown_handoff.py "nodamp=hp26_ld_c[fghi]*:nodamp" "stock=hp26_ld_c[ghi]*:stock" | tee -a "$LOG"
  [ -f "$CAMPAIGN_DIR/STOP_CI" ] && break
  say "block $i locomotionSafe checks over its runs (trips = any branch; spikes = absorbed grazes): $(legv_count "$CAMPAIGN_DIR/wkc26_ld_ci$i.csv" "$CAMPAIGN_DIR/hp26_ld_ci$i.csv")"
  tier "$i"
done
say "chain CI done (STOP_CI seen after block $i)"
