...
| **What now limits the course** | 🔬 **open, and it is a real dynamic limit rather than a bug with a trigger** — OPEN-28. Eight hypotheses eliminated with their own controls (overspeed, host stall, turning, gait changes, any isolated corner 30–180° at 0/66, long straights, chaining, approach speed). The live lead: read backward from the E-stop, **ride height goes first** — `z` sags ~0.05 m *before* the pitch moves, with `kin_z` tracking it, so the legs really shorten. Loss of support, not an estimator artifact. Accumulating falls at n≈100 to test it properly |
