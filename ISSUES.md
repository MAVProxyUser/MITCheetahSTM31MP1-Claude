...
| 8 | **The archive: thin it, move it, or leave it — and there is now a DEADLINE.** | 50 GB, fully compacted, no retention. **Measured 2026-09-16: 35 GB free at 12:26 → 28 GB at 19:25, i.e. 1.0 GB/h, 24 GB/day** — four times the ~6 GB/day on record, because today ran back-to-back campaigns and one chain had `DUMP=1`. At that rate the harness's own DISK GUARD warns (20 GB) in **~8 h** and STOPS the chains (8 GB) in **~20 h**; the guard is verified live in `open28_subcourse.sh` and exits 3 rather than writing into a full disk, so the failure is clean but it ENDS the queued work. The remaining chains run `DUMP=0`, which removes the bridge dumps. Note also that compaction is packing correctly and **still not freeing space, because three `com.apple.os.update-*` APFS snapshots pin the originals** — which is decision #6, so #6 and #8 are the same decision seen twice. I have not deleted anything: thinning is destructive and it is your call. | OPEN-35 |
