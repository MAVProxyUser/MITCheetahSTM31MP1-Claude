#!/bin/bash
# Chain CP (2026-09-16 07:45): the same question on the course where the envelope decision is LIVE.
...
# absorbed-spike lines and the trips in the block's archived ctrl logs, until STOP_CP.
...
# same, and the tier, DUMP=1, pooled scores each block, until STOP_CP (RULE ONE). If the
...
tier(){ local t="$1"; tm_wait; say "$t: fast suite tier"
  python3 unittests/test_validated_missions.py --fast > "$CAMPAIGN_DIR/suite_fast_20260916_cp$t.log" 2>&1
  say "$t suite exit $?: $(grep -cE "^  PASS" "$CAMPAIGN_DIR/suite_fast_20260916_cp$t.log") PASS, $(grep -cE "^  FAIL" "$CAMPAIGN_DIR/suite_fast_20260916_cp$t.log") FAIL"
  sleep 20; wait_idle
}
ycount(){
  awk -F, 'FNR>1 && $7!="" { n[$2]++; if($4=="PASS") p[$2]++; s[$2]+=$7; if($7+0>mx[$2]) mx[$2]=$7+0; if($17!=""){t[$2]+=$17; tn[$2]++} }
    END { for (a in n) printf "    %s: %d/%d PASS  peak pitch mean %.1f  MAX %.1f deg  |  lap %.1f s\n", a, p[a]+0, n[a], s[a]/n[a], mx[a], (tn[a]?t[a]/tn[a]:0) }' "$@" 2>/dev/null; }
...
PREV="${PREV_CHAIN_PID:-52992}"
LOG="$CAMPAIGN_DIR/open28_chaincp.log"
say "chain CP pid $$: waiting for chain CO (pid $PREV) to exit"
while kill -0 "$PREV" 2>/dev/null && ps -p "$PREV" -o command= | grep -q "campaign_chain_20260912co[.]sh"; do sleep 30; done
rm -f "$CAMPAIGN_DIR/STOP_CP"
...
wait_idle; tm_wait
COURSES=wkc_finals ARMS="recipe:" run wkc26_vcruise_probe 1 2.6
Q=$(awk -F, 'NR>1{print ($4=="PASS") ? "QUALIFIED: run "$11" verdict PASS" : "NOT QUALIFIED: run "$11" verdict "$4}' "$CAMPAIGN_DIR/wkc26_vcruise_probe.csv" 2>/dev/null | tail -1)
say "probe $Q"
...
tier 0
i=0
while [ ! -f "$CAMPAIGN_DIR/STOP_CP" ]; do
  i=$((i+1)); wait_idle; sleep 30; wait_idle; tm_wait; say "block $i: rig idle (phase $(phase)) - binary $(md5 -q host-run/mit_ctrl_sim | cut -c1-8)"
  COURSES=wkc_finals ARMS="$A" run "wkc26_vcruise_cp$i" 6 2.6
  say "wkc peak pitch and verdicts per arm, pooled over CP (what the 2.7 rung costs in margin):"
  ycount "$CAMPAIGN_DIR"/wkc26_vcruise_cp*.csv | tee -a "$LOG"
  tier "$i"
done
say "chain CP done (STOP_CP seen after block $i)"
