...
|---|---|---|---|
| **foot friction** | ~~mu = 2.0~~ **feet mu 0.6 in every world in the tree (re-checked 2026-09-10, 10 world files); the ground plane carries NO friction element, so the pair is 0.6 vs the engine default 1.0 combined by DART's rule (min, if memory serves - unverified here)** | URDF says **0.6**; rubber on concrete ~0.8-1.0 | The 2.0 era is gone - the row was never re-read after the speedway proto replaced it. The `concrete` surface kind sets BOTH sides to 0.90 (`apply_terrain` + `apply_surface_feet`), which is the honest way to test grip: OPEN-33 A/Bs `flat` vs `concrete`. If anything the shipped pair is the URDF's own figure, not a flattering one. |
