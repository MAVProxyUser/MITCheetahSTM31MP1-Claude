...
| `open28_subcourse.sh` arm tokens | `ARMS="name:A=1,B=2"` (comma = several variables in one arm); two tokens are CONSUMED by the harness rather than passed to the controller: `SPEED=<m/s>` (that arm's cruise, so a speed ladder interleaves) and `TERRAIN=<kind>` (that arm's `--terrain`: flat, concrete, grass, … so a friction A/B interleaves). A `COURSES` entry with a colon (`dash:100`) is a raw slot spec, a bare name is `course:<name>`. `DUMP=1` records the bridge's 100 Hz command stream per run |
| `campaign_chain_YYYYMMDD.sh` | a multi-campaign chain is a FILE in git, not an inline `bash -c`: it waits on pids/markers, never edits a running script, deploys only on an idle conductor, and runs campaigns sequentially so `campaign_claim` never sees an overlap. `campaign_chain_20260910.sh` is the template |
| `open31_score.py` / `item5_score.py` | the joint-limit and contact-gate A/B scorers (verdicts + the arm's own instrumentation: heartbeat `clamps=/stops=` counters and calf range from the dumps; `[ESTERR]` velocity error and the gate's `vetoed X of Y` line) |
