...
# 8 reps, interleaved, after chain AV (pid 32102; AV took the slot when the
# wp08 traces made the slew the more decision-relevant lever).
set -u
cd "$(dirname "${BASH_SOURCE[0]}")/../.."
. gazebo/tools/paths.sh
PREV="${PREV_CHAIN_PID:-32102}"
...
say "chain AU pid $$: waiting for chain AV (pid $PREV) to exit"
while kill -0 "$PREV" 2>/dev/null && ps -p "$PREV" -o command= | grep -q "campaign_chain_20260912av.sh"; do sleep 30; done
