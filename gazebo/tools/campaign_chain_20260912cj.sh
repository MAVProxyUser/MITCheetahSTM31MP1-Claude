#!/bin/bash
# Chain CJ (2026-09-15 22:30): ship the HELD-SAMPLE GATE on locomotionSafe (ISSUES OPEN-39 22:25).
...
# absorbed-spike lines and the trips in the block's archived ctrl logs, until STOP_CJ.
...
# same, and the tier, DUMP=1, pooled scores each block, until STOP_CJ (RULE ONE). If the
...
tier(){ local t="$1"; tm_wait; say "$t: fast suite tier"
  python3 unittests/test_validated_missions.py --fast > "$CAMPAIGN_DIR/suite_fast_20260915_cj$t.log" 2>&1
  say "$t suite exit $?: $(grep -cE "^  PASS" "$CAMPAIGN_DIR/suite_fast_20260915_cj$t.log") PASS, $(grep -cE "^  FAIL" "$CAMPAIGN_DIR/suite_fast_20260915_cj$t.log") FAIL"
  sleep 20; wait_idle
}
...
PREV="${PREV_CHAIN_PID:-91669}"
LOG="$CAMPAIGN_DIR/open39_chaincj.log"
say "chain CJ pid $$: waiting for chain CI (pid $PREV) to exit"
while kill -0 "$PREV" 2>/dev/null && ps -p "$PREV" -o command= | grep -q "campaign_chain_20260912ci[.]sh"; do sleep 30; done
rm -f "$CAMPAIGN_DIR/STOP_CJ"
wait_idle; sleep 30; wait_idle; tm_wait
OLD=$(md5 -q host-run/mit_ctrl_sim | cut -c1-8)
say "deploy: building mit_ctrl_sim (held-sample gate, CTRL_LEG_HELD_GATE=1) - deployed binary now $OLD"
DEPLOYED=0
if cmake --build host-build -j2 --target mit_ctrl_sim > "$CAMPAIGN_DIR/build_cj.log" 2>&1; then
  say "build ok ($(grep -c 'warning' "$CAMPAIGN_DIR/build_cj.log") warnings); deploying through deploy_host.sh"
  if bash gazebo/deploy_host.sh >> "$CAMPAIGN_DIR/build_cj.log" 2>&1; then DEPLOYED=1; say "deploy ok: binary $OLD -> $(md5 -q host-run/mit_ctrl_sim | cut -c1-8)"; else say "DEPLOY FAILED (see build_cj.log) - the old binary stays"; fi
else
  say "BUILD FAILED (see build_cj.log) - the old binary stays"
fi
# probe the served recipe once and qualify on the snapshot + dump (archived at the run's end)
wait_idle; tm_wait
COURSES=wkc_finals ARMS="nodamp:" run wkc26_ld_cj_probe 1 2.6
Q=$(python3 - <<'PY'
import csv, sys
...
import os
rows = list(csv.DictReader(open(os.path.join(os.environ.get("CHEETAH_DATA", os.path.expanduser("~/Desktop/Cheetah/rundata")), "campaigns", "wkc26_ld_cj_probe.csv"))))
...
tier 0
i=0
while [ ! -f "$CAMPAIGN_DIR/STOP_CJ" ]; do
  i=$((i+1)); wait_idle; sleep 30; wait_idle; tm_wait; say "block $i: rig idle (phase $(phase)) - binary $(md5 -q host-run/mit_ctrl_sim | cut -c1-8)"
  COURSES=wkc_finals ARMS="$A" run "wkc26_ld_cj$i" 6 2.6
  say "wkc lie-down, pooled:"; python3 gazebo/tools/liedown_peak.py "nodamp=wkc26_ld_c[fghij]*:nodamp" "stock_new=wkc26_ld_c[ghij]*:stock" "stock_all=wkc26_l*:recipe" | tee -a "$LOG"
  say "wkc hand-off joints (dumps):"; python3 gazebo/tools/liedown_handoff.py "nodamp=wkc26_ld_c[fghij]*:nodamp" "stock=wkc26_ld_c[ghij]*:stock" | tee -a "$LOG"
  [ -f "$CAMPAIGN_DIR/STOP_CJ" ] && break
  COURSES=hp_gap20 ARMS="$A" run "hp26_ld_cj$i" 6 2.6
  say "hairpin lie-down, pooled:"; python3 gazebo/tools/liedown_peak.py "nodamp=hp26_ld_c[fghij]*:nodamp" "stock_new=hp26_ld_c[ghij]*:stock" "stock_all=hp26_l*:recipe" | tee -a "$LOG"
  say "hairpin hand-off joints (dumps):"; python3 gazebo/tools/liedown_handoff.py "nodamp=hp26_ld_c[fghij]*:nodamp" "stock=hp26_ld_c[ghij]*:stock" | tee -a "$LOG"
  [ -f "$CAMPAIGN_DIR/STOP_CJ" ] && break
  say "block $i locomotionSafe checks over its runs (trips = any branch; spikes = absorbed grazes): $(legv_count "$CAMPAIGN_DIR/wkc26_ld_cj$i.csv" "$CAMPAIGN_DIR/hp26_ld_cj$i.csv")"
  tier "$i"
done
say "chain CJ done (STOP_CJ seen after block $i)"
