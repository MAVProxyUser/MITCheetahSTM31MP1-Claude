    ...
    for line in open(p, errors="replace"):
        ...
        if m:
            ...
            if (vT[0] ** 2 + vT[1] ** 2) ** 0.5 > 1.0:
                dvx.append(abs(vE[0] - vT[0])); dvy.append(abs(vE[1] - vT[1]))
            continue
        g = GATE.search(line)
        if g:
            gate = (int(g.group(1)), int(g.group(2)), float(g.group(3)))
    return dict(n=len(dvx), dvx=dvx, dvy=dvy, gate=gate, log=os.path.basename(p))
...
def main(paths):
    for path in paths:
        ...
        arms = {}
        for r in rows:
            a = arms.setdefault(r["course"], dict(n=0, passed=0, dvx=[], dvy=[], gate=[], runs=0, missing=0))
            a["n"] += 1; a["passed"] += r["verdict"] == "PASS"
            s = score_run(r["run_id"])
            if not s or s["n"] == 0:
                a["missing"] += 1; continue
            a["runs"] += 1
            a["dvx"].append(statistics.mean(s["dvx"])); a["dvy"].append(statistics.mean(s["dvy"]))
            a["gate"].append(s["gate"][2] if s["gate"] else None)
        print("== %s (%d rows)" % (os.path.basename(path), len(rows)))
        print("  %-10s %7s  %8s %8s  %8s %8s  %s" % ("arm", "PASS", "|dvx|mean", "p90", "|dvy|mean", "p90", "gate veto % (runs with ESTERR / missing)"))
        for name, a in sorted(arms.items()):
            gv = [g for g in a["gate"] if g is not None]
            print("  %-10s %3d/%-3d  %8.3f %8.3f  %8.3f %8.3f  %s  (%d / %d)" % (
                name, a["passed"], a["n"],
