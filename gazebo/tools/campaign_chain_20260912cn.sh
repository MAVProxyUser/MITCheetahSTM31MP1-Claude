#!/bin/bash
# Chain CN (2026-09-16 04:25): the hairpin RE-ACCELERATION cap, OPEN-28 run 7994's lever. That fall
...
# absorbed-spike lines and the trips in the block's archived ctrl logs, until STOP_CN.
...
# same, and the tier, DUMP=1, pooled scores each block, until STOP_CN (RULE ONE). If the
...
tier(){ local t="$1"; tm_wait; say "$t: fast suite tier"
  python3 unittests/test_validated_missions.py --fast > "$CAMPAIGN_DIR/suite_fast_20260916_cn$t.log" 2>&1
  say "$t suite exit $?: $(grep -cE "^  PASS" "$CAMPAIGN_DIR/suite_fast_20260916_cn$t.log") PASS, $(grep -cE "^  FAIL" "$CAMPAIGN_DIR/suite_fast_20260916_cn$t.log") FAIL"
  sleep 20; wait_idle
}
ycount(){ shift 0
...
PREV="${PREV_CHAIN_PID:-4327}"
LOG="$CAMPAIGN_DIR/open28_chaincn.log"
say "chain CN pid $$: waiting for chain CM (pid $PREV) to exit"
while kill -0 "$PREV" 2>/dev/null && ps -p "$PREV" -o command= | grep -q "campaign_chain_20260912cm[.]sh"; do sleep 30; done
rm -f "$CAMPAIGN_DIR/STOP_CN"
...
wait_idle; tm_wait
COURSES=hp_gap20 ARMS="recipe:" run hp26_reacc_probe 1 2.6
Q=$(awk -F, 'NR>1{print ($4=="PASS") ? "QUALIFIED: run "$11" verdict PASS" : "NOT QUALIFIED: run "$11" verdict "$4}' "$CAMPAIGN_DIR/hp26_reacc_probe.csv" 2>/dev/null | tail -1)
say "probe $Q"
...
A="recipe: reacc24:WP_REACCEL_VMAX=2.4"
...
tier 0
i=0
while [ ! -f "$CAMPAIGN_DIR/STOP_CN" ]; do
  i=$((i+1)); wait_idle; sleep 30; wait_idle; tm_wait; say "block $i: rig idle (phase $(phase)) - binary $(md5 -q host-run/mit_ctrl_sim | cut -c1-8)"
  COURSES=hp_gap20 ARMS="$A" run "hp26_reacc_cn$i" 6 2.6
  say "hairpin peak pitch per arm, pooled over CN (the quantity the mechanism consumes):"
  ycount "$CAMPAIGN_DIR"/hp26_reacc_cn*.csv | tee -a "$LOG"
  tier "$i"
done
say "chain CN done (STOP_CN seen after block $i)"
