#!/bin/bash
# Compact archived shm_trace snapshots older than AGE_H hours (default 6) in
...
#   usage: archive_compact.sh            (AGE_H=6 by default - was 24 until
...
echo "$(date '+%F %T') compacted $n snapshots older than ${AGE_H:-6} h ($fail failed): $((before/1024)) MB -> $((after/1024)) MB; disk free $(df -h "$DIR" | awk 'NR==2{print $4}')" | tee -a "$LOG"
