#!/bin/bash
# Chain BK (2026-09-12 21:55): the rung below and the rung above on the served
# recipe - wkc_finals 2.4 x10 (the old shipping rung, now with 16 deg peaks)
# and hp_gap20 2.7 x10 (5/5 under the 2.0 budget in chain BB). After chain BJ (pid 76652).
set -u
...
PREV="${PREV_CHAIN_PID:-76652}"
LOG="$CAMPAIGN_DIR/open38_chainbk.log"
say "chain BK pid $$: waiting for chain BJ (pid $PREV) to exit"
while kill -0 "$PREV" 2>/dev/null && ps -p "$PREV" -o command= | grep -q "campaign_chain_20260912bj[.]sh"; do sleep 30; done
wait_idle; sleep 30; wait_idle; tm_wait; say "rig idle (phase $(phase)) - binary $(md5 -q host-run/mit_ctrl_sim | cut -c1-8)"
COURSES=wkc_finals ARMS="recipe:" run wkc24_recipe_n10 10 2.4
COURSES=hp_gap20 ARMS="recipe:" run hp27_recipe_n10 10 2.7
say "chain BK done"
