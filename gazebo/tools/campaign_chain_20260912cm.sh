#!/bin/bash
# Chain CM (2026-09-16 03:20): the same measurement with an INSTRUMENT that can see it. CL block 1
...
# absorbed-spike lines and the trips in the block's archived ctrl logs, until STOP_CM.
...
# same, and the tier, DUMP=1, pooled scores each block, until STOP_CM (RULE ONE). If the
...
tier(){ local t="$1"; tm_wait; say "$t: fast suite tier"
  python3 unittests/test_validated_missions.py --fast > "$CAMPAIGN_DIR/suite_fast_20260916_cm$t.log" 2>&1
  say "$t suite exit $?: $(grep -cE "^  PASS" "$CAMPAIGN_DIR/suite_fast_20260916_cm$t.log") PASS, $(grep -cE "^  FAIL" "$CAMPAIGN_DIR/suite_fast_20260916_cm$t.log") FAIL"
  sleep 20; wait_idle
}
ycount(){ local csv="$1"
  awk -F, 'NR>1 && $21!="" { n[$2]++; s[$2]+=$21; if($21+0>mx[$2]) mx[$2]=$21+0; if(mn[$2]==""||$21+0<mn[$2]) mn[$2]=$21+0 }
    END { for (a in n) printf "    %s: n=%d  max |p(1)| per run: min %d  mean %.1f  MAX %d mm   (limit for this arm: %s mm)\n", a, n[a], mn[a], s[a]/n[a], mx[a], (a=="y024"?"240":"300") }' "$csv" 2>/dev/null
  awk -F, 'NR>1 && $21=="" {b++} END {if (b) printf "    (%d rows without the column - a binary older than the instrument)\n", b}' "$csv" 2>/dev/null; }
...
PREV="${PREV_CHAIN_PID:-76357}"
LOG="$CAMPAIGN_DIR/open39_chaincm.log"
say "chain CM pid $$: waiting for chain CL (pid $PREV) to exit"
while kill -0 "$PREV" 2>/dev/null && ps -p "$PREV" -o command= | grep -q "campaign_chain_20260912cl[.]sh"; do sleep 30; done
rm -f "$CAMPAIGN_DIR/STOP_CM"
wait_idle; sleep 30; wait_idle; tm_wait
OLD=$(md5 -q host-run/mit_ctrl_sim | cut -c1-8)
say "deploy: building mit_ctrl_sim (leg_y_max instrument: max |p(1)| per second) - deployed binary now $OLD"
DEPLOYED=0
if cmake --build host-build -j2 --target mit_ctrl_sim > "$CAMPAIGN_DIR/build_cm.log" 2>&1; then
  say "build ok ($(grep -c 'warning' "$CAMPAIGN_DIR/build_cm.log") warnings); deploying through deploy_host.sh"
  if bash gazebo/deploy_host.sh >> "$CAMPAIGN_DIR/build_cm.log" 2>&1; then DEPLOYED=1; say "deploy ok: binary $OLD -> $(md5 -q host-run/mit_ctrl_sim | cut -c1-8)"; else say "DEPLOY FAILED (see build_cm.log) - the old binary stays"; fi
else
  say "BUILD FAILED (see build_cm.log) - the old binary stays"
fi
# probe the served recipe once and qualify on the snapshot + dump (archived at the run's end)
wait_idle; tm_wait
COURSES=wkc_finals ARMS="nodamp:" run wkc26_ply_cm_probe 1 2.6
Q=$(awk -F, 'NR>1{print ($4=="PASS") ? "QUALIFIED: run "$11" verdict PASS" : "NOT QUALIFIED: run "$11" verdict "$4}' "$CAMPAIGN_DIR/wkc26_ply_cm_probe.csv" 2>/dev/null | tail -1)
say "probe $Q"
...
tier 0
i=0
while [ ! -f "$CAMPAIGN_DIR/STOP_CM" ]; do
  i=$((i+1)); wait_idle; sleep 30; wait_idle; tm_wait; say "block $i: rig idle (phase $(phase)) - binary $(md5 -q host-run/mit_ctrl_sim | cut -c1-8)"
  COURSES=wkc_finals ARMS="$A" run "wkc26_ply_cm$i" 6 2.6
  say "wkc lateral excursion per arm (the quantity the limit acts on):"
  ycount "$CAMPAIGN_DIR/wkc26_ply_cm$i.csv" | tee -a "$LOG"
  tier "$i"
done
say "chain CM done (STOP_CM seen after block $i)"
