...
| **f** | **CYCLE CAP: keep the guard's action for a one-off trip, go advisory only once the pattern proves itself a cycle** (`CTRL_LOCO_UNSAFE_CYCLE_CAP`, with `CTRL_LOCO_UNSAFE_CYCLE_GAP_TICKS` = 250 separating a cycle from two unrelated trips — inside the cycle successive trips are 2–3 ticks apart) | **Strictly more conservative than (d)**, which never transitions at all above a speed. This one still aborts locomotion on a genuine single trip — the behaviour the guard was written for — and only stops re-transitioning after N consecutive trip/recover pairs. What motivates it is the 2026-09-17 finding that **the hazard is the SUSTAINED cycle, not the trip**: at the shipped trigger, runs with ≤ 2 cycles survived **6 of 6** while runs with ≥ 31 cycles fell **7 of 11**, and entry height did not discriminate (FAIL median 0.295 m vs PASS 0.293 m). The data supports any cap comfortably inside 2–31; **5 is the obvious first value**. The cost, stated plainly: it does not remove the hazard, it BOUNDS it — a capped run still eats N cycles of bleed before the cap engages, where (d) eats none. It is the option to reach for if (d)'s blanket suppression is judged too much guard to give up | **written 2026-09-17, `CTRL_LOCO_UNSAFE_CYCLE_CAP`, default 0 = off. NOT BUILT OR DEPLOYED** — the TU passes `-fsyntax-only` against the real `mit_ctrl_sim` flags (0 errors, all 113 warnings from `third-party/eigen`) and nothing else. Needs a build in an idle gap and a probe run before any campaign, per the probe-a-default rule. Unmeasured |
