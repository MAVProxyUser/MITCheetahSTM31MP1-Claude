...
| **The finish-line fall (2026-09-06, closed)** | ✅ **fixed, and it was neither the brake nor the settle.** A course could clear all 16 waypoints and then go over, ~half the time. Between the planner's end brake and `K_BALANCE_STAND` the robot stands **stopped, in `LOCOMOTION`, at zero commanded velocity** for up to 2.6 s — the wait after the stop ramp watched *speed only*, and a toppling body is not still, so it burned its full backstop. `zeroVelHold`'s own note had already measured that regime as taking a stop from ~1-in-3 tips to 7-of-8. The confirm loop now ends on **attitude** as well as speed, and `settleOnFeet()` replaces the blind 1.5 s sleep at all four `BALANCE_STAND` sites. Interleaved, ramp intact: **0/10 vs 4/8**, p = 0.0076 against a three-block pooled control of 18/37; whole-run PASS 9/14 vs 2/14, p = 0.0183. Two wrong causes were chased first and both are written up in `ISSUES.md` CLOSED (was OPEN-27) |
| **What now limits the course** | ✅ **closed 2026-09-10 — it was the simulation harness, not the robot** (OPEN-28). macOS holds loopback UDP datagrams 20–45 ms about once a second; a freeze landing on a stance exchange froze the swing command through the flip. The link is now unix-domain datagrams, host stragglers are swept at campaign start, and `wkc_finals` at 1.9 runs **24/24 PASS**. The finish-line tips (OPEN-30) are closed too: the stop ramp seeds from the body's speed, 15/15 clean. Sixteen hypotheses were tested against a harness that was the cause; the record keeps them |
| **MPPI on the Mac GPU** | 🔬 **stage 1 answered, and it is discouraging** — OPEN-29. Sampling cannot cold-start this problem *at all*: 0/4096 samples feasible at every sigma, because the origin sits on the lower boundary of all 100–200 friction-cone rows. Warm-started it works and matches — but does not beat — simply reusing the previous solve. On MPS it costs 8.6 ms at K=16k (10× faster than CPU, launch-bound below 4 k) against qpOASES's 0.6–1.7 ms here. The problem is convex, which is sampling's worst case. And none of it reaches the board, which has no compute GPU |
...
| **Run data** | ✅ lives in `$CHEETAH_DATA` (default `<repo>/../rundata`), **never `/tmp`** — one definition in `gazebo/paths.py` / `gazebo/tools/paths.sh`. Raw rings are distilled to per-run summaries + 50 Hz descent windows (`gazebo/tools/distill.py`): most recently **668 snapshots, 20 GB → 31.6 MB** plus 40 complete rings kept at full resolution |
| **Campaign harness** | ✅ `gazebo/tools/run_queue.sh` (every job under `timeout`, a failure never stops the queue), `campaign_truth.sh` (arms alternated **every rep**, ground truth + contact labels per run, instrument proven before the campaign spends the night), `rig_watchdog.sh` (idle rig alarms on a `pgrep`, not a phase string). **Identity guards**: `dump_snapshot(expect_run_id=…)` refuses to archive a ring belonging to a different run — shm outlives its writer, so an aborted run leaves the previous one's data sitting there and it was recorded six times as six data points before this existed. The expectation comes from the runner's own stdout, the one place that cannot be stale. `campaign_health_gate` treats consecutive `NONE` verdicts as a host problem and stops rather than writing fiction |
