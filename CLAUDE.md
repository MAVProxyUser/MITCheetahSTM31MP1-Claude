...
*The shipped trot envelope at N=10..40 (2026-09-12): straight 3.0, box 2.6 — and the
hairpin 2.5 / wkc_finals 2.4 rows are DOUBLE-LAP numbers (ISSUES OPEN-38, found 15:40):
on every course with a collinear reversal the follower's nearest-index tie-break
U-turned the dog 4 m before the vertex, the waypoint layer froze there, the planner
...
|---|---|---|---|
| **foot friction** | ~~mu = 2.0~~ **feet mu 0.6 in every world in the tree (re-checked 2026-09-10, 10 world files); the ground plane carries NO friction element, so the pair is 0.6 vs the engine default 1.0 combined by DART's rule, which is the MINIMUM (`ContactSurface.cpp` v6.13.2: `params.mPrimaryFrictionCoeff = (std::min)(frictionCoeffA, frictionCoeffB)`; restitution is the product) - so the shipped pair is exactly 0.6** | URDF says **0.6**; rubber on concrete ~0.8-1.0 | The 2.0 era is gone - the row was never re-read after the speedway proto replaced it. The `concrete` surface kind sets BOTH sides to 0.90 (`apply_terrain` + `apply_surface_feet`), which is the honest way to test grip: OPEN-33 A/Bs `flat` vs `concrete`. If anything the shipped pair is the URDF's own figure, not a flattering one. Measured 2026-09-10/11: concrete adds +9.6° of roll at the hairpin's reversal apex (6/6 pairs) because grip lets the body turn as fast as the follower asks; `WP_VSLEW=1.0` (now in the `course:` recipe) takes 12–17° of that roll back on concrete and 11° of pitch off the reversal exit at wkc 2.2, and lifts wkc's usable cruise from 2.0 to 2.4. |
...
| **joint velocity** | ~~unbounded~~ **bounded at 30.1 rad/s in every world joint** (re-checked 2026-09-10: `<velocity>30.1</velocity>` on all 12 joints of `go1_speedway.sdf`; the hip reaches it in fast swings) | finite, and the binding constraint for a fast swing | The earlier "unbounded" was never re-verified against the world file - a record error, not a sim gap. What the sim still lacks is the TORQUE-SPEED curve (back-EMF derating), which is the actuator-dynamics row above. |
...
| **IMU** | Gazebo IMU; orientation is an exact pass-through by default, and since 2026-09-10 the bridge can perturb it (`BRIDGE_ORI_NOISE_DEG`: bias random walk τ≈5 s + 30 % white) | real VectorNav/DroneCAN noise and bias | Measured (OPEN-33): 0.5° RMS costs the 1.9 hairpin +2.5° of peak roll at most (6/6 vs 6/6, not significant). The DroneCAN/Madgwick path the real machine runs is a larger dose (1.5–3°), untested. |
...
**And WHICH STEP KILLS — read the correction below before using this.** **SUPERSEDED: the fold-as-killer reading here was REFUTED by its own intervention** (option (e), `CTRL_RECOVER_FOLD_VMAX`, 2026-09-16: the manipulation took — entry folds 180 → 51 — and the outcome was stock's, so suppressing folds while the cycle continues changes nothing). The 93 % association is the DEPTH CONFOUND the caveat below names: folding is what RecoveryStand does below 0.20 m, so a folded robot is one that has already bled out. **The bleed is the killer; the fold is its marker.** And the "duration does not discriminate" half of this paragraph is an artefact of ITS corpus — it pooled all eras, and before the 5-tick debounce a ONE-TICK spike started the cycle, so every run cycled heavily and duration had no variance left to show. In era C, where only genuinely marginal excursions trip at all, cycle count discriminates hard (median 68 for falls vs 2 for survivors) because it is the UPSTREAM driver: at 2.29 mm a cycle, cycle count is simply how you reach the depth that kills. Crossing 0.20 m in the era-C corpus goes with 6 of 7 falls against 1 of 4 above it, replicating the association — and, per the intervention, still as a depth marker rather than a cause. The original paragraph follows, for the record: 
