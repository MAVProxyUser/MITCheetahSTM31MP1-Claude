...
| 8 | **The archive: thin it, move it, or leave it — with a measured deadline.** | 50 GB, fully compacted, no retention. Free space today: **35.0 GB at 12:26 → 28.0 at 19:25 → 27.15 at 20:55.** That is **1.00 GB/h** over the first window, which included the one chain writing bridge dumps, and **0.57 GB/h** since, with `DUMP=0` chains only — instantaneously ~0.21 GB/h during a tier, because compaction packs as fast as the runs write. **CORRECTED: at 0.57 GB/h the harness WARNS (20 GB) in ~13 h and STOPS the chains (8 GB) in ~34 h** — I earlier said 8 h and 20 h from the dump-inclusive average, which was too pessimistic, and every remaining chain runs `DUMP=0`. The guard is verified live in `open28_subcourse.sh` and exits 3 rather than writing into a full disk, so the failure is clean but it ENDS the queued work. Compaction is packing correctly and **still not freeing space, because three `com.apple.os.update-*` APFS snapshots pin the originals** — so #6 and #8 are one decision. I have deleted nothing: thinning is destructive and it is your call. | OPEN-35 |
