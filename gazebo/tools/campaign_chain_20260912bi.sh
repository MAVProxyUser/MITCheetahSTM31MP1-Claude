#!/bin/bash
# Chain BI (2026-09-12 21:55): the shipping rungs of the honest courses on the
# SERVED recipe (WP_ALAT=2.0 since 21:02) at N=10, for the record and for
# OPEN-30's finish-tip rate (1 in 6 on the hairpin in chain BG). After chain BH (pid 61889).
set -u
...
PREV="${PREV_CHAIN_PID:-61889}"
LOG="$CAMPAIGN_DIR/open38_chainbi.log"
say "chain BI pid $$: waiting for chain BH (pid $PREV) to exit"
while kill -0 "$PREV" 2>/dev/null && ps -p "$PREV" -o command= | grep -q "campaign_chain_20260912bh[.]sh"; do sleep 30; done
wait_idle; sleep 30; wait_idle; tm_wait; say "rig idle (phase $(phase)) - binary $(md5 -q host-run/mit_ctrl_sim | cut -c1-8)"
COURSES=wkc_finals ARMS="recipe:" run wkc26_recipe_n10 10 2.6
COURSES=hp_gap20 ARMS="recipe:" run hp26_recipe_n10 10 2.6
say "chain BI done"
